...
ok then how we can implement this in login
...
we have a user_auth to handle the logic between login and sign up. we can change the state of the signState with that function. 
event.preventDefaut is to prevent the page reload. 

what is this onChange ? 
it is a event handler in react that triggers whenever the value in input field changes(even when a user types in there)
